...
public class SharedTabBuilder {
    ...
    public SharedTabBuilder(MontoyaApi api, Database db, MessageTableModel messageTableModel, JTabbedPane jTabbedPane, JTabbedPane jTabbedPaneA, JTabbedPane jTabbedPaneB) {
        this.api = api;
        this.db = db;
    ...
    public DatatablePanel generateTabWithData(LinkedHashMap<String, Object> dataMap, HttpRequest httpRequest) {
        ...
        for (String i : dataMap.keySet()) {
            if (i.equals("Value")) {
                component = new DatatablePanel(api, db, messageTableModel, columnNameB, dataMap.get(i), httpRequest, i);
                jTabbedPaneB.addTab(i, component);
            } else if (i.equals("Current Value")) {
                String name = i.replace("Current ", "");
                component = new DatatablePanel(api, db, messageTableModel, columnNameB, dataMap.get(i), httpRequest, name);
                jTabbedPaneA.addTab(name, component);
            } else if (i.equals("Current Param")) {
                String name = i.replace("Current ", "");
                component = new DatatablePanel(api, db, messageTableModel, columnNameA, dataMap.get(i), httpRequest, name);
                jTabbedPaneA.addTab(name, component);
            } else if (i.contains("All")) {
                component = new DatatablePanel(api, db, messageTableModel, columnNameA, dataMap.get(i), httpRequest, i);
